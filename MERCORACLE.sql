...
CREATE OR REPLACE PACKAGE PK_EMPLEADO IS
    ...
    PROCEDURE CREAR_EMPLEADO (EMP EMPLEADO%ROWTYPE);
...
CREATE OR REPLACE PACKAGE BODY PK_EMPLEADO IS
    ...
    PROCEDURE ASIGNAR_CUENTA(DNI VARCHAR2, NOMBRE_USUARIO VARCHAR2) IS
        SENT_CREAR_USUARIO VARCHAR2(100); 
        SENT_ASIGNAR_ROL   VARCHAR2(100);
        EMP                EMPLEADO%ROWTYPE;
    BEGIN
        SENT_CREAR_USUARIO := 'CREATE USER ' || NOMBRE_USUARIO || ' IDENTIFIED BY BD;';
